  ...
  return (
    <div id="stats-reports-container" className="space-y-6">
      ...
      <div id="stats-filter-bar" className="bg-slate-900 text-white p-4 rounded-2xl border border-slate-800 shadow-md flex flex-col sm:flex-row gap-4 items-center justify-between no-print">
        ...
        <div className="flex gap-2 w-full sm:w-auto shrink-0 justify-end">
          <button
            ...
            className="px-3.5 py-2 text-sm border border-slate-700 hover:bg-slate-800 text-slate-200 rounded-xl transition-all flex items-center gap-1.5 cursor-pointer"
            title="Xuất file văn bản nháp Word"
          >
            <FileText className="w-4 h-4 text-blue-400" /> Báo cáo Word
          </button>
          <button
          ...
          >
            <FileSpreadsheet className="w-4 h-4 text-emerald-400" /> Xuất Excel Học kỳ
          </button>
          <button
      ...
      <div id="printable-report-card" className="bg-slate-900 text-slate-150 p-6 rounded-2xl border border-slate-800 shadow-md space-y-6 print:bg-white print:text-black print:border-none print:shadow-none">
        ...
        <div className="text-center space-y-1.5 border-b border-slate-800 pb-5 print:border-slate-100">
          <h4 className="text-xs uppercase tracking-widest font-extrabold text-slate-400 font-display print:text-slate-500">{schoolName}</h4>
          <h2 className="text-lg md:text-2xl font-black text-white font-display print:text-black">BÁO CÁO ĐÁNH GIÁ CHẤT LƯỢNG MÔN TIN HỌC</h2>
          <p className="text-xs text-slate-300 print:text-slate-600">
            Lớp học: <span className="font-bold text-white print:text-black">{targetClass?.name || 'Toàn trường'}</span> · Kỳ báo cáo: <span className="font-bold">{selectedSemester}</span> · Sĩ số: <span className="font-bold">{classStudents.length} học sinh</span>
          </p>
        </div>
        ...
        {totalAssessmentsCount === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="p-5 bg-slate-800 rounded-2xl border border-slate-750 flex flex-col items-center justify-center space-y-4 print:bg-slate-50 print:text-slate-900 print:border-slate-100">
              <h3 className="font-bold text-white text-sm flex items-center gap-1.5 print:text-black">
                <BarChart2 className="w-4.5 h-4.5 text-blue-400 print:text-blue-600" /> Phân bổ kết quả hoàn thành bài học
              </h3>
              ...
              <div className="relative w-44 h-44 flex items-center justify-center">
                <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                  {/* Outer circle track */}
                  <circle cx="50" cy="50" r="40" fill="transparent" className="stroke-slate-700 print:stroke-slate-200" strokeWidth="12" />
                ...
                <div className="absolute text-center">
                  <span className="text-xl font-extrabold text-white print:text-black">{excellentRate + goodRate}%</span>
                  <p className="text-[10px] text-slate-450 print:text-slate-500 font-semibold uppercase mt-0.5">Tỷ lệ đạt</p>
                </div>
              </div>

              {/* Legend list */}
              <div className="w-full text-xs space-y-1.5 text-slate-300 print:text-slate-650">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-bold text-white print:text-black">{excellentRate}% ({excellentCount} lượt)</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="flex items-center gap-1.5 font-medium"><span className="w-3 h-3 rounded bg-blue-500"></span> Hoàn thành</span>
                  <span className="font-bold text-white print:text-black">{goodRate}% ({goodCount} lượt)</span>
                </div>
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-bold text-white print:text-black">{incompleteRate}% ({incompleteCount} lượt)</span>
                </div>
              </div>
            </div>

            {/* Right box: Progress and specific categories */}
            <div className="p-5 bg-slate-800 rounded-2xl border border-slate-750 space-y-5 flex flex-col justify-center print:bg-slate-50 print:text-slate-900 print:border-slate-100">
              <h3 className="font-bold text-white text-sm print:text-black font-display">Các tiêu chuẩn phát triển</h3>
              
              <div>
                <div className="flex justify-between text-xs font-semibold mb-1 text-slate-300 print:text-slate-700">
                  <span>😊 Thái độ tích cực hoạt bát</span>
                  <span>{positiveAttitudeRate}% tích cực</span>
                </div>
                <div className="w-full bg-slate-700 print:bg-slate-200 h-2 rounded-full overflow-hidden">
              ...
              <div className="p-3 bg-slate-850/60 text-slate-300 text-xs rounded-xl flex gap-2 border border-slate-700/50 items-start print:bg-blue-50 print:text-blue-800 print:border-blue-100">
                <Info className="w-4 h-4 shrink-0 mt-0.5 text-blue-400 print:text-blue-500" />
